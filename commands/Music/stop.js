...
module.exports = class extends MusicCommand {
    ...
    async run(msg) {
        ...
        if (!music.queue || !music.queue.length) return msg.send(`***There are no songs in the queue at the moment.***`);

        const vc = music.voiceChannel ? music.voiceChannel.members.size <= 4 : true;
        if (await msg.hasAtLeastPermissionLevel(3) || vc) {
            await music.destroy();
            return msg.send(`***Queue cleared, leaving voice channel.***`);
        } else {
            return msg.send(`***There are members in the VC right now, use skip instead!***`);
        }
    }
