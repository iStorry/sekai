...
module.exports = class extends MusicCommand {
    ...
    async run(msg) {
        ...
        if (!music.queue.length) return msg.send(msg.language.get("MUSIC_NO_SONGS_IN_QUEUE"));

        const pages = new RichDisplay(new MessageEmbed()
            .setTitle(`${msg.language.get("MUSICIF_QUEUE_TITLE")}`)
            .setAuthor(`Sekai - Music Queue`, "https://i.imgur.com/IS8hX4t.png")
        ...
        pages.run(await msg.send(`${this.client.emotes.loading} ${msg.language.get("MUSICIF_QUEUE_LOADING")}`), {
            time: 120000,
