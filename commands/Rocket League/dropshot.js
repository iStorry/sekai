...
module.exports = class extends Command {
    ...
    async run(msg, [platform, username]) {
        ...
        let dropshot = stats.filter(data => data.label == "Dropshot");
        if (!dropshot[0]) return msg.send(`** No Dropshot Data**`);
        /// Rough
        let rankLabel = dropshot[0].subLabel.split("]");
        let rankMain = rankLabel[1] ? rankLabel[1] + ` (${dropshot[0].displayValue})` : "Unknown";
        /// Design Config
        let fontColor = "#FFFFFF";
        try {

        const bg = await this.getBase();
        const rank = await this.ranksIcon(dropshot[0].subLabel);
        const img = await new Canvas(400, 100)
            ...
            .addText(dropshot[0].subLabel, 35, 140)
            .toBufferAsync();
