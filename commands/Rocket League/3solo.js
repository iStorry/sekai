...
module.exports = class extends Command {
    ...
    constructor(...args) {
        super(...args, {
            ...
            permissionLevel: 0,
            runIn: ['text', 'dm'],
            description: 'Solo Standard Rank.',
    ...
    async run(msg, [platform, username]) {
        ...
        let standard = stats.filter(data => data.label == "Ranked Solo Standard 3v3");
        if (!standard[0]) return msg.send(`** No Solo Standard Rank Data**`);
        /// Rough
        let rankLabel = standard[0].subLabel.split("]");
        let rankMain = rankLabel[1] ? rankLabel[1] + ` (${standard[0].displayValue})` : "Unknown";
        /// Design Config
        let fontColor = "#FFFFFF";
        try {

        const bg = await this.getBase();
        const rank = await this.ranksIcon(standard[0].subLabel);
        const img = await new Canvas(400, 100)
            ...
            .addText(standard[0].subLabel, 35, 140)
            .toBufferAsync();
