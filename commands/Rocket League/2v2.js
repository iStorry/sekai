...
module.exports = class extends Command {
    ...
    constructor(...args) {
        super(...args, {
            name: '2v2',
            aliases: ['2s', '2v2', 'double'],
            permissionLevel: 0,
            runIn: ['text', 'dm'],
            description: 'Double Rank.',
    ...
    async run(msg, [platform, username]) {
        ...
        let double = stats.filter(data => data.label == "Ranked Doubles 2v2");
        if (!double[0]) return msg.send(`** No Double Rank Data**`);
        /// Rough
        let rankLabel = double[0].subLabel.split("]");
        let rankMain = rankLabel[1] ? rankLabel[1] + ` (${double[0].displayValue})` : "Unknown";
        /// Design Config
        let fontColor = "#FFFFFF";
        try {

        const bg = await this.getBase();
        const rank = await this.ranksIcon(double[0].subLabel);
        const img = await new Canvas(400, 100)
            ...
            .addText(double[0].subLabel, 35, 140)
            .toBufferAsync();
